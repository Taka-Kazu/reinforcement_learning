...
class PPONet(object):
  ...
  def _build_net(self, name, trainable):
    with tf.variable_scope(name):
      ...
      l1 = tf.layers.dense(cell_out, NUM_HIDDENS[0], tf.nn.relu, trainable=trainable, name="l1")
      #l2 = tf.layers.dense(l1, NUM_HIDDENS[1], tf.nn.relu, trainable=trainable, name="l2")
      l3 = tf.layers.dense(l1, NUM_HIDDENS[2], tf.nn.relu, trainable=trainable, name="l3")
      alpha = tf.layers.dense(l3, NUM_ACTIONS, tf.nn.softplus, trainable=trainable, name="actor_alpha")
      beta = tf.layers.dense(l3, NUM_ACTIONS, tf.nn.softplus, trainable=trainable, name="actor_beta")
      value = tf.layers.dense(l3, 1, trainable=trainable, name="critic")
