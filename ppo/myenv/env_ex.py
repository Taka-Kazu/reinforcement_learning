...
class MyEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.max_distance = math.sqrt(2) * self.WORLD_SIZE
    self.NUM_LIDAR = 36
    self.NUM_KERNEL = 20
    self.NUM_TARGET = 3
  ...
  def reset(self):
    ...
    while True:
      ...
      if not self.target_available(self.target):
        break
    self.MAP = reset_map(self.MAP_SIZE)
    self.dis = math.sqrt((self.target[0]-self.pose[0])**2 + (self.target[1]-self.pose[1])**2)
  ...
  def step(self, action):
    #pose update
    self.pose[0] = self.pose[0] + action[0]*math.cos(self.pose[2])*self.DT
    self.pose[1] = self.pose[1] + action[0]*math.sin(self.pose[2])*self.DT
...
#    reward += 1./(200.*np.pi)*angle_diff(np.arctan2((self.pose[1]-self.target[1]),(self.pose[0]-self.target[0])),self.pose[2])
  ...
  def observe(self):
    ...
    print("===observation===")
    print(self.last_lidar_data)
    print("{0}[s]".format(-start_time+time.time()))
    #pose
    observation[self.NUM_LIDAR] = math.sqrt((self.target[0]-self.pose[0])**2 +(self.target[1]-self.pose[1])**2)
    theta = math.atan2((self.target[1]-self.pose[1]),(self.target[0]-self.pose[0]))
    theta = angle_diff(theta,self.pose[2])
    observation[self.NUM_LIDAR+1] = math.sin(theta)
    observation[self.NUM_LIDAR+2] = math.cos(theta)
  ...
  def raycasting(self, pose, angle):
    ...
    x1 = int((pose[0]+self.RANGE_MAX * math.cos(pose[2]+angle))/self.MAP_RESOLUTION)
    y1 = int((pose[1]+self.RANGE_MAX * math.sin(pose[2]+angle))/self.MAP_RESOLUTION)
    steep = False
    ...
    if steep:
      ...
      if not self.is_movable_grid(pose_):
        return math.sqrt((x-x0)**2 + (y-y0)**2) * self.MAP_RESOLUTION
    else:
      pose_ = [x,y,0]
      if not self.is_movable_grid(pose_):
        return math.sqrt((x-x0)**2 + (y-y0)**2) * self.MAP_RESOLUTION
  ...
  def target_available(self, pose):
    min_dis = self.RANGE_MAX
    margin_collision = self.robot_radius * 0.1
    NUM_CHECK = 36
